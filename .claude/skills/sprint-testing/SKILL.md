...
| Test-user credentials + roles | NOT REQUIRED (no auth) | This project's API is public (`AllowAny`) — no creds, no roles, no tokens. Skipped unless the target gains auth. |
...
| OpenAPI MCP (schema read-only) | N/A (no spec published) | The target publishes no OpenAPI spec (drf-spectacular not installed); the `openapi` MCP is disabled. Contracts live in hand-written `api/schemas/*` facades. Execution is curl's job (next row). |
| Public API curl (no token) | SCOPE — when API execution is in scope | Requests run via **curl** against the public API (`AllowAny`) — no auth header, no token. Canon: `agentic-qa-core/references/api-testing-doctrine.md`. |
